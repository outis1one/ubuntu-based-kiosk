#!/bin/bash
################################################################################
###   Ubuntu Based Kiosk (UBK) v0.9.16         ###
################################################################################
#
# RELEASE v0.9.16 - Fix Rotation Timing Issues
#
# What's in v0.9.16:
# - CRITICAL FIX: Removed early return that blocked rotation
#   * v0.9.15 had userRecentlyActive check that blocked ALL rotation for 60s after interaction
#   * This caused sites to never rotate even when they had time set
#   * Removed lines that returned early when user was recently active
# - Rotation now works correctly with interaction tracking
#   * Sites rotate on schedule unless user has interacted with that specific site
#   * Interaction flag properly blocks rotation only for the interacted site
#   * Other sites continue to rotate normally
# - Inactivity popup now appears correctly
#   * Popup only shows after 1 minute of inactivity on sites user interacted with
#   * Doesn't appear on home screen unless user interacted with home screen
#   * Properly respects media playback (no popup when media playing)
# - All previous v0.9.15 features still work
#   * User can extend time with 15/30/60/120 minute options
#   * Auto-return to rotation after 15 seconds if no response
#   * Media check every 3 seconds (good performance)
#
# What was in v0.9.15 (had bugs):
...
# - BUG: Rotation blocked for ALL sites for 60s after ANY interaction (FIXED in v0.9.16)
#
# What's in v0.9.14:
...
delete_site() {
    ...
    echo
    read -r -p "Enter number to delete (0=cancel): " del_num
    if [[ "$del_num" =~ ^[0-9]+$ ]] && [[ "$del_num" -ge 1 ]] && [[ "$del_num" -le "${#URLS[@]}" ]]; then
...
edit_site_url() {
    ...
    echo " ═══ EDIT SITE URL ═══"
    echo
    echo "Edit which site?"
    for idx in "${!URLS[@]}"; do
        local dur_display="${DURS[$idx]}s"
        [[ "${DURS[$idx]}" == "-1" ]] && dur_display="hidden"
        [[ "${DURS[$idx]}" == "0" ]] && dur_display="manual"
        echo "  $((idx+1)). ${URLS[$idx]} ($dur_display)"
    done
    echo
    read -r -p "Enter number to edit (0=cancel): " edit_num

    if [[ "$edit_num" =~ ^[0-9]+$ ]] && [[ "$edit_num" -ge 1 ]] && [[ "$edit_num" -le "${#URLS[@]}" ]]; then
        local edit_idx=$((edit_num-1))
        echo
        echo "Editing site #$edit_num:"
        echo "Current URL: ${URLS[$edit_idx]}"
        echo "Current Duration: ${DURS[$edit_idx]}s"
        echo "Current User: ${USERS[$edit_idx]:-none}"
        echo

        read -r -p "New URL [${URLS[$edit_idx]}]: " new_url
        new_url="${new_url:-${URLS[$edit_idx]}}"
        ...
        echo
        read -r -p "Change duration too? (y/n) [n]: " change_dur
        local new_dur="${DURS[$edit_idx]}"

        if [[ "$change_dur" =~ ^[Yy]$ ]]; then
            echo
            echo "Duration Guide:"
            echo "  • 0 = Manual only (won't auto-rotate)"
            echo "  • 1-999 = Auto-rotates every X seconds"
            echo "  • -1 = Hidden tab (F10 to access)"
            echo
            read -r -p "Duration in seconds [${DURS[$edit_idx]}]: " new_dur
            new_dur="${new_dur:-${DURS[$edit_idx]}}"
        fi

        echo
        read -r -p "Change authentication? (y/n) [n]: " change_auth
        local new_user="${USERS[$edit_idx]}"
        local new_pass="${PASSES[$edit_idx]}"

        if [[ "$change_auth" =~ ^[Yy]$ ]]; then
            echo
            read -r -p "Username (leave blank for none) [${USERS[$edit_idx]}]: " new_user
            new_user="${new_user:-${USERS[$edit_idx]}}"
        ...
        URLS[$edit_idx]="$new_url"
        DURS[$edit_idx]="$new_dur"
        USERS[$edit_idx]="$new_user"
        PASSES[$edit_idx]="$new_pass"

        log_success "Site #$edit_num updated"
        echo "  URL: $new_url"
        echo "  Duration: ${new_dur}s"
        [[ -n "$new_user" ]] && echo "  Auth: $new_user"
        echo
    else
        echo "Cancelled"
    fi
...
reorder_sites() {
    ...
    echo " ═══ REORDER SITES ═══"
    echo
    echo "Sites are rotated in the order listed below."
    echo "Use this to change the rotation order."
    echo
    ...
    while true; do
        ...
        for idx in "${!URLS[@]}"; do
            ...
            [[ "${DURS[$idx]}" == "0" ]] && dur_display="manual"
            local home_mark=""
            [[ "$HOME_TAB_INDEX" == "$idx" ]] && home_mark=" [HOME]"
            echo "  $((idx+1)). ${URLS[$idx]} ($dur_display)$home_mark"
        done
        echo

        read -r -p "Select site to move (0=done): " move_num

        if [[ "$move_num" == "0" ]]; then
            log_success "Reordering complete"
            return
        fi

        if ! [[ "$move_num" =~ ^[0-9]+$ ]] || [[ "$move_num" -lt 1 ]] || [[ "$move_num" -gt "${#URLS[@]}" ]]; then
        ...
        local move_idx=$((move_num-1))
        echo
        echo "Moving: ${URLS[$move_idx]}"
        echo
        echo "Options:"
        start_menu
        [[ "$move_idx" -gt 0 ]] && add_option "Move up (swap with site #$move_num)"
        [[ "$move_idx" -lt $((${#URLS[@]}-1)) ]] && add_option "Move down (swap with site #$((move_num+2)))"
        add_option "Jump to specific position"
        show_menu_prompt

        local reorder_choice=$choice
        local menu_offset=0

        # Handle menu numbering based on available options
        if [[ "$move_idx" -gt 0 ]]; then
            case "$reorder_choice" in
                1)  # Move up
                    local swap_idx=$((move_idx-1))

                    # Swap URLs
                    local temp_url="${URLS[$move_idx]}"
                    URLS[$move_idx]="${URLS[$swap_idx]}"
                    URLS[$swap_idx]="$temp_url"

                    # Swap durations
                    local temp_dur="${DURS[$move_idx]}"
                    DURS[$move_idx]="${DURS[$swap_idx]}"
                    DURS[$swap_idx]="$temp_dur"

                    # Swap users
                    local temp_user="${USERS[$move_idx]}"
                    USERS[$move_idx]="${USERS[$swap_idx]}"
                    USERS[$swap_idx]="$temp_user"

                    # Swap passwords
                    local temp_pass="${PASSES[$move_idx]}"
                    PASSES[$move_idx]="${PASSES[$swap_idx]}"
                    PASSES[$swap_idx]="$temp_pass"

                    # Update HOME_TAB_INDEX if needed
                    if [[ "$HOME_TAB_INDEX" == "$move_idx" ]]; then
                        HOME_TAB_INDEX=$swap_idx
                    elif [[ "$HOME_TAB_INDEX" == "$swap_idx" ]]; then
                        HOME_TAB_INDEX=$move_idx
                    fi

                    log_success "Moved up"
                    echo
                    continue
                    ;;
            esac
        ...
        if [[ "$move_idx" -lt $((${#URLS[@]}-1)) ]]; then
            if [[ "$reorder_choice" == "$((menu_offset+1))" ]]; then  # Move down
                local swap_idx=$((move_idx+1))
        ...
        if [[ "$reorder_choice" == "$((menu_offset+1))" ]]; then  # Jump to position
            echo
            read -r -p "Jump to position (1-${#URLS[@]}): " new_pos

            if ! [[ "$new_pos" =~ ^[0-9]+$ ]] || [[ "$new_pos" -lt 1 ]] || [[ "$new_pos" -gt "${#URLS[@]}" ]]; then
            ...
            local new_idx=$((new_pos-1))

            if [[ "$new_idx" == "$move_idx" ]]; then
            ...
            unset 'URLS[$move_idx]'
            unset 'DURS[$move_idx]'
            unset 'USERS[$move_idx]'
            unset 'PASSES[$move_idx]'
            URLS=("${URLS[@]}")
            DURS=("${DURS[@]}")
            USERS=("${USERS[@]}")
            PASSES=("${PASSES[@]}")

            # Adjust new_idx if moving forward
            if [[ "$new_idx" -gt "$move_idx" ]]; then
                new_idx=$((new_idx-1))
            fi

            # Insert at new position
            URLS=("${URLS[@]:0:$new_idx}" "$temp_url" "${URLS[@]:$new_idx}")
            DURS=("${DURS[@]:0:$new_idx}" "$temp_dur" "${DURS[@]:$new_idx}")
            USERS=("${USERS[@]:0:$new_idx}" "$temp_user" "${USERS[@]:$new_idx}")
            PASSES=("${PASSES[@]:0:$new_idx}" "$temp_pass" "${PASSES[@]:$new_idx}")

            # Update HOME_TAB_INDEX if needed
            if [[ "$HOME_TAB_INDEX" == "$move_idx" ]]; then
                HOME_TAB_INDEX=$new_idx
            elif [[ "$move_idx" -lt "$HOME_TAB_INDEX" ]] && [[ "$HOME_TAB_INDEX" -le "$new_idx" ]]; then
                HOME_TAB_INDEX=$((HOME_TAB_INDEX-1))
            elif [[ "$move_idx" -gt "$HOME_TAB_INDEX" ]] && [[ "$HOME_TAB_INDEX" -ge "$new_idx" ]]; then
                HOME_TAB_INDEX=$((HOME_TAB_INDEX+1))
            fi

            log_success "Moved to position $new_pos"
            echo
            continue
        elif [[ "$reorder_choice" == "0" ]]; then
            echo
            continue
        fi
...
configure_lockout() {
    echo
    echo " ═══ SESSION LOCKOUT CONFIGURATION ═══"
    echo
    echo "Session lockout protects your kiosk with a password after inactivity."
    echo
    echo "How it works:"
    echo "  • After X minutes of inactivity, screen locks (goes black)"
    echo "  • Shows 'Session Locked' message with password prompt"
    echo "  • Enter password to unlock and resume"
    echo "  • Password is separate from sudo/system password"
    echo

    if sudo -u "$KIOSK_USER" test -f "$CONFIG_PATH" 2>/dev/null; then
        LOCKOUT_ENABLED=$(sudo -u "$KIOSK_USER" jq -r '.lockoutEnabled // "false"' "$CONFIG_PATH" 2>/dev/null)
        LOCKOUT_TIMEOUT=$(sudo -u "$KIOSK_USER" jq -r '.lockoutTimeout // 1800' "$CONFIG_PATH" 2>/dev/null)
    fi

    if [[ "$LOCKOUT_ENABLED" == "true" ]]; then
        local timeout_min=$((LOCKOUT_TIMEOUT / 60))
        echo "Current: Enabled (${timeout_min} min timeout)"
    else
        echo "Current: Disabled"
    fi
    echo
    ...
    echo "Options:"
    start_menu
    add_option "Enable/Change Lockout"
    add_option "Disable Lockout"
    show_menu_prompt
    lockout_choice=$choice

    case "$lockout_choice" in
        1)
            echo
            read -r -p "Lockout timeout in minutes [30]: " timeout_min
            timeout_min="${timeout_min:-30}"
            LOCKOUT_TIMEOUT=$((timeout_min * 60))

            echo
            echo "Set a password for unlocking the kiosk:"
            echo "(This password is separate from your system password)"
            echo
            while true; do
                ...
                if [[ "$pass1" == "$pass2" ]]; then
                    if [[ -n "$pass1" ]]; then
                        ...
                        log_success "Session lockout enabled (${timeout_min} min timeout)"
                        break
                    else
        ...
        2)
            ...
            LOCKOUT_PASSWORD=""
            LOCKOUT_TIMEOUT=1800
            log_success "Session lockout disabled"
            ;;
        0)
            echo "Cancelled"
            ;;
...
save_config() {
    ...
    local lockout_enabled_json="false"
    [[ "$LOCKOUT_ENABLED" == "true" ]] && lockout_enabled_json="true"
    ...
    jq -n \
      ...
      --argjson lockoutEnabled "$lockout_enabled_json" \
      --arg lockoutPassword "${LOCKOUT_PASSWORD:-}" \
      --argjson lockoutTimeout "${LOCKOUT_TIMEOUT:-1800}" \
      '{unit:$unit,autoswitch:$autoswitch,enableTouch:$enableTouch,dualSwipe:$dualSwipe,swipeMode:$swipeMode,allowNavigation:$allowNavigation,homeTabIndex:$homeTabIndex,inactivityTimeout:$inactivityTimeout,lockoutEnabled:$lockoutEnabled,lockoutPassword:$lockoutPassword,lockoutTimeout:$lockoutTimeout,tabs:[]}' > "$tmp"
...
let lockoutPassword='';
let lockoutTimeout=1800000;  // 30 minutes in milliseconds
let lockoutWindow=null;
let sessionLocked=false;
let lastLockoutCheck=Date.now();
...
function loadConfig(){
  try{
    ...
    lockoutTimeout=(config.lockoutTimeout||1800)*1000;
    ...
    console.log('[CONFIG] Lockout enabled:',lockoutEnabled);
    if(lockoutEnabled){
      console.log('[CONFIG] Lockout timeout:',lockoutTimeout/1000,'seconds');
    }
...
function startMasterTimer(){
  ...
  masterTimer=setInterval(()=>{
    ...
    // 5. SITE ROTATION
    // v0.9.16: Rotation happens on schedule UNLESS user interacted with that specific site
    // The userInteractedWithCurrentSite flag blocks rotation only for the interacted site
    ...
    // 6. INACTIVITY CHECK
    // v0.9.16: Show popup ONLY on sites where user has interacted after inactivity timeout
    ...
    // 7. LOCKOUT CHECK (session lock after extended inactivity)
    ...
    if(lockoutEnabled&&!sessionLocked){
      ...
      if(idleTime>lockoutTimeout*0.75){
        if(Math.floor(idleTime/30000)!==Math.floor((idleTime-1000)/30000)){
          const idleMinutes=Math.floor(idleTime/60000);
          const idleSeconds=Math.floor((idleTime%60000)/1000);
          const timeoutMinutes=Math.floor(lockoutTimeout/60000);
          console.log('[LOCKOUT] ⏱️ IDLE: '+idleMinutes+'m '+idleSeconds+'s / '+timeoutMinutes+'m (lockout)');
        }
      }

      if(idleTime>=lockoutTimeout){
        console.log('[LOCKOUT] 🔒 Timeout reached - locking session');
        showLockout();
      }
    }
  },1000);
...
function showLockout(){
  if(lockoutWindow&&!lockoutWindow.isDestroyed())return;
  if(sessionLocked)return;

  console.log('[LOCKOUT] 🔒 Showing lockout screen');
  sessionLocked=true;

  // Close any open prompts or keyboards
  if(promptWindow&&!promptWindow.isDestroyed()){
    promptWindow.close();
    promptWindow=null;
  }
  ...
  lockoutWindow.loadFile(path.join(__dirname,'lockout.html'));
...
function hideLockout(){
  ...
  lockoutWindow=null;
  sessionLocked=false;
  lastUserInteraction=Date.now();  // Reset inactivity timer
  lastLockoutCheck=Date.now();     // Reset lockout timer
  console.log('[LOCKOUT] 🔓 Session unlocked');
}
...
function createWindow(){
  ...
  ipcMain.on('unlock-session',(event,password)=>{
    console.log('[LOCKOUT] Unlock attempt received');
    if(!lockoutEnabled||!sessionLocked){
      console.log('[LOCKOUT] Not locked or lockout disabled');
      return;
    }

    if(password===lockoutPassword){
      console.log('[LOCKOUT] ✓ Password correct');
      if(lockoutWindow&&!lockoutWindow.isDestroyed()){
        lockoutWindow.webContents.send('unlock-success');
      }
      setTimeout(()=>{
        hideLockout();
      },500);
    }else{
      console.log('[LOCKOUT] ✗ Password incorrect');
      if(lockoutWindow&&!lockoutWindow.isDestroyed()){
        lockoutWindow.webContents.send('unlock-failed');
      }
    }
  });
...
############################start-lockout-html##############################
###########################################################################
    sudo -u "$KIOSK_USER" tee "$KIOSK_DIR/lockout.html" > /dev/null <<'LOCKOUTHTML'
<!DOCTYPE html>
<html>
<head>
  <meta charset="UTF-8">
...
<body>
  <div class="lockout-container">
    ...
    <div class="message">
      Your kiosk session has been locked due to inactivity.<br>
      Enter your password to continue.
    </div>

    <form class="password-form" onsubmit="return unlockSession(event)">
      <input
        type="password"
        id="password"
    ...
    <div class="info">
      ℹ️ The session automatically locks after inactivity<br>
  ...
  <script>
    ...
    function unlockSession(event) {
      ...
      console.log('[LOCKOUT] Attempting to unlock...');
      ipcRenderer.send('unlock-session', password);
      return false;
    }
  ...
  document.addEventListener('touchend',e=>{
    if(e.changedTouches.length>=1){
      ...
      if(fingerCount===3&&absY>SWIPE_THRESHOLD&&absX<SWIPE_TOLERANCE&&deltaY<0){
        ...
        if(target&&isTextInput(target)&&!keyboardVisible){
          keyboardAutoClosedThisSession=false;
          const now=Date.now();
          if(now-lastKeyboardRequest>KEYBOARD_REQUEST_THROTTLE){
            lastKeyboardRequest=now;
...
# Keep screen on forever - watchdog
(
  while true; do
...
# Audio watchdog - checks every 3 minutes
(
  while true; do
    sleep 30
    ...
    # Check if PTT service is running
...
CURRENT_ELECTRON=$(npm list electron --depth=0 2>/dev/null | grep electron@ | cut -d'@' -f2 | cut -d' ' -f1)
echo "Current: v${CURRENT_ELECTRON}"
echo ""
...
if [[ "$CHECK_UPDATES" =~ ^[Yy]$ ]]; then
  ...
  if [ -z "$LATEST_ELECTRON" ]; then
    echo "✗ Could not fetch latest version"
  else
    echo "Latest: v${LATEST_ELECTRON}"
    echo ""
    ...
    if [ "$CURRENT_ELECTRON" = "$LATEST_ELECTRON" ]; then
      ...
      CURRENT_MAJOR=$(echo "$CURRENT_ELECTRON" | cut -d'.' -f1)
      LATEST_MAJOR=$(echo "$LATEST_ELECTRON" | cut -d'.' -f1)
      
      echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
      if [ "$LATEST_MAJOR" -gt "$CURRENT_MAJOR" ]; then
        echo "⚠️  MAJOR VERSION UPDATE"
        echo "This is a major version change (v$CURRENT_MAJOR → v$LATEST_MAJOR)"
        echo "May contain breaking changes!"
        echo ""
        echo "Breaking changes to check:"
        echo "  • API changes in Electron $LATEST_MAJOR"
        echo "  • Node.js version compatibility"
        echo "  • Chromium engine updates"
        echo ""
        echo "Review: https://www.electronjs.org/docs/latest/breaking-changes"
      else
        echo "Minor/patch update (v$CURRENT_ELECTRON → v$LATEST_ELECTRON)"
        echo "Should be safe to update"
      fi
      echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
      echo ""
      
      read -p "Update to v${LATEST_ELECTRON}? (y/n): " DO_UPDATE
      ...
      if [[ "$DO_UPDATE" =~ ^[Yy]$ ]]; then
        ...
        if [ $? -eq 0 ]; then
          ...
          echo ""
          read -p "Restart kiosk now? (y/n): " RESTART_NOW
          if [[ "$RESTART_NOW" =~ ^[Yy]$ ]]; then
            echo "Restarting..."
            sudo systemctl restart lightdm
          else
            echo "Restart later with: sudo systemctl restart lightdm"
          fi
        else
          echo "✗ Update failed"
        fi
...
core_menu() {
    while true; do
        ...
        case "$choice" in
            ...
            3) configure_navigation_security; save_config ;;
            4) configure_sites ;;
            5) configure_wifi ;;
