#!/bin/bash
################################################################################
###   Ubuntu Based Kiosk (UBK) v0.9.5-7          ###
################################################################################
#
# RELEASE v0.9.5-7 - CRITICAL Inactivity Lockout Extension Fix
#
# What's NEW in v0.9.5-7:
# - CRITICAL FIX: Inactivity lockout now respects time extensions
#   * Lockout timer was ignoring pause button and manual site timeout extensions
...
# - All fixes from v0.9.5-6 included
#
# ROOT CAUSE:
# The checkLockoutTimer() function only checked lockoutActivityTime and didn't
# respect inactivityExtensionUntil. This meant:
# - User clicks "15 minutes" on pause button
# - Site rotation/timeout is blocked ✓
# - But inactivity lockout still triggered after configured time ✗
#
# FIX: Now checkLockoutTimer() checks inactivityExtensionUntil first and skips
# lockout check during extension period, just like rotation and home return do.
#
