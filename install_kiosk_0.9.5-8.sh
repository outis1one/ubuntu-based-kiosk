#!/bin/bash
################################################################################
###   Ubuntu Based Kiosk (UBK) v0.9.5-8          ###
################################################################################
#
# RELEASE v0.9.5-8 - CRITICAL Lockout Activity Timer Reset Fix
#
# What's NEW in v0.9.5-8:
# - CRITICAL FIX: Extensions now properly reset lockout activity timer
#   * Pause button extensions now reset lockoutActivityTime (was missing)
#   * Manual site timeout extensions now reset lockoutActivityTime (was missing)
#   * Prevents lockout from counting idle time DURING extension period
#   * Lockout timer now truly starts fresh after extension selected
# - All fixes from v0.9.5-7 included
#
# ROOT CAUSE (v0.9.5-7 was incomplete):
# In v0.9.5-7, checkLockoutTimer() correctly skipped lockout during extensions.
# BUT the extension handlers didn't reset lockoutActivityTime when setting extension:
#
# showInactivityPrompt() when user selects time (line 4632):
#   ✓ Set inactivityExtensionUntil
#   ✓ Set lastUserInteraction
#   ✓ Set siteStartTime
#   ✗ Did NOT set lockoutActivityTime
#
# showPauseDialog() when user selects time (line 4680):
#   ✓ Set inactivityExtensionUntil
#   ✓ Set lastUserInteraction
#   ✓ Set siteStartTime
#   ✗ Did NOT set lockoutActivityTime
...
# - Lockout configured for 30 minutes
# - User clicks "15 minutes" extension
# - Extension blocks lockout check ✓ (v0.9.5-7 fix)
# - But lockoutActivityTime still shows 10 minutes ago ✗
# - Extension expires after 15 minutes
# - v0.9.5-7 resets lockoutActivityTime to now ✓
# - User is now considered idle for 0 minutes ✓
# - Lockout happens 30 minutes later ✓
#
# But this meant lockout never actually enforced the configured timeout properly
# because extensions kept resetting it at expiration instead of at selection.
#
# FIX (v0.9.5-8):
# Now BOTH handlers reset lockoutActivityTime when extension is selected:
# - Lines 4637-4641: Manual site inactivity prompt extension
# - Lines 4684-4688: Pause button extension
#
# This means lockout timer considers the user "active" when they select an
# extension, not when the extension expires. More intuitive behavior.
#
...
function showLockoutScreen(){
  ...
  // Note: No need to hide views - fullscreen lockout window will cover them
