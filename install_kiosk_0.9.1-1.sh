#!/bin/bash
################################################################################
###   Ubuntu Based Kiosk (UBK) v0.9.1-1          ###
################################################################################
#
# RELEASE v0.9.1-1 - Home Return Fix for Timed Sites
#
# What's NEW in v0.9.1-1:
# - FIXED: Home return popup now works on timed rotation sites
# - Previously, timed sites would auto-rotate but never show the inactivity prompt
# - Now both auto-rotation AND home return work together on all site types
#
    ...
    if(homeTabIndex>=0&&!showingHidden){
      ...
      if(homeViewIdx>=0&&currentIndex!==homeViewIdx){
        ...
        if(inactivityExtensionUntil>0&&now<inactivityExtensionUntil){
          ...
          // Extension expired - clear it and reset rotation timer
          console.log('[HOME] ⏰ Extension expired - resetting rotation timer and checking timeout');
          inactivityExtensionUntil=0;
