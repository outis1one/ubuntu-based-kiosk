#!/bin/bash
################################################################################
###   Ubuntu Based Kiosk (UBK) v0.9.5-6          ###
################################################################################
#
# RELEASE v0.9.5-6 - CRITICAL Boot Password Sequence Fix
#
# What's NEW in v0.9.5-6:
# - CRITICAL SECURITY FIX: Boot password now required BEFORE showing any websites
...
#   * Fixed sequence: was attachView() then check boot flag, now check boot flag FIRST
# - SECURITY: Limited power menu available when locked out
#   * Ctrl+Alt+Del when locked shows only Shutdown/Restart (no Reload option)
#   * Prevents bypassing boot password via Reload
#   * Still allows emergency shutdown/restart without password
# - All fixes from v0.9.5-5 included
#
# RELEASE v0.9.5-5 - Boot Password Bypass Fix
#
# What was in v0.9.5-5:
# - Added isLockedOut check to showPowerMenu() function
# - Prevented power menu access during lockout states
#
