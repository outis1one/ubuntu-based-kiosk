#!/bin/bash
################################################################################
###   Ubuntu Based Kiosk (UBK) v0.9.2-4          ###
################################################################################
#
# RELEASE v0.9.2-4 - Fixed Electron Update
#
# What's NEW in v0.9.2-4:
# - FIXED: Manual Electron update no longer bails/exits script
# - Properly detects current installed Electron version
# - Auto-backup before upgrade with rollback instructions
# - Shows both current and latest versions before update
# - Safe update process with error handling
#
# RELEASE v0.9.2-3 - Time Extension Tab Switch Fix
#
# What's in v0.9.2-3:
# - FIXED: Time extensions now clear when switching tabs (any method)
# - Extensions are site-specific - 15min on Site 1 won't affect Site 2
# - Switching tabs (swipe/keyboard/auto-rotation) clears active extension
#
# RELEASE v0.9.2-2 - CRITICAL Extension Preservation Fix
#
# What's in v0.9.2-2:
# - CRITICAL FIX: User activity no longer cancels active extensions
# - Extension timer now persists for full selected duration (15/30/60/120 min)
# - Bug fixed: markActivity() was clearing inactivityExtensionUntil
# - Extensions now work correctly - mouse/touch doesn't cancel pause
#
# RELEASE v0.9.2-1 - CRITICAL Timer Fix
#
# What's in v0.9.2-1:
# - Extension check moved to HIGHEST PRIORITY (before user activity check)
# - When paused, ALL timers blocked (rotation + inactivity)
# - Extension expiry automatically resets all timers
# - Better logging: [EXTENSION] shows pause status every 30s
#
...
save_config() {
    ...
    jq -n \
      ...
      '{unit:$unit,autoswitch:$autoswitch,enableTouch:$enableTouch,dualSwipe:$dualSwipe,swipeMode:$swipeMode,allowNavigation:$allowNavigation,homeTabIndex:$homeTabIndex,inactivityTimeout:$inactivityTimeout,tabs:[]}' > "$tmp"
...
function markActivity(){
  ...
  // BUGFIX v0.9.2-2: Don't cancel active extensions on user activity!
  // User explicitly set an extension via pause button - preserve it.
  // Extensions are only cleared when:
  //   1. They expire naturally (handled in timer check)
...
function startMasterTimer(){
  ...
  masterTimer=setInterval(()=>{
    ...
    // 5. EXTENSION/PAUSE CHECK - HIGHEST PRIORITY
    // If user has paused via pause button or inactivity prompt, block ALL timers
    if(inactivityExtensionUntil>0&&now<inactivityExtensionUntil){
      ...
      if(Math.floor(now/30000)!==Math.floor((now-1000)/30000)){
        ...
        console.log('[EXTENSION] ⏸ Paused - '+remMin+'m '+remSec+'s remaining (rotation and inactivity blocked)');
      }
      return;  // Block rotation and inactivity checks during extension
    }else if(inactivityExtensionUntil>0&&now>=inactivityExtensionUntil){
      // Extension expired - clear it and reset timers
      console.log('[EXTENSION] ⏰ Extension expired - resuming normal operation');
      inactivityExtensionUntil=0;
      siteStartTime=now;  // Reset rotation timer when extension expires
      lastUserInteraction=now;  // Reset inactivity timer
    }

    // 6. USER ACTIVITY CHECK
      ...
      if(currentTabIdx>=0&&tabs[currentTabIdx]){
        ...
        if(siteDuration>0){
          ...
          if(timeOnSite>=siteDuration*1000){
            rotateToNextSite();
            // Removed return; to allow HOME RETURN CHECK to run on timed sites
          }
        }
      }
    }

    // 8. HOME RETURN CHECK (manual sites only)
    if(homeTabIndex>=0&&!showingHidden){
      ...
      if(homeViewIdx>=0&&currentIndex!==homeViewIdx&&currentTabIdx>=0&&tabs[currentTabIdx]){
        ...
        // Extension check is now handled at top level (section 5)
        ...
        if(Math.floor(idleTime/15000)!==Math.floor((idleTime-1000)/15000)){
          ...
          const timeoutSeconds=Math.floor((inactivityTimeout%60000)/1000);
...
function attachView(i){
  ...
  // v0.9.2-3 FIX: Clear time extension when switching sites
...
function showPauseDialog(){
  ...
  ipcMain.once('pause-time-selected',(event,minutes)=>{
    ...
    if(minutes===0){
      ...
      // Set extension time - timers reset when extension expires (see EXTENSION CHECK)
      const now=Date.now();
      inactivityExtensionUntil=now+(minutes*60*1000);
      console.log('[PAUSE] Extended for '+minutes+' min until: '+new Date(inactivityExtensionUntil).toLocaleTimeString());
      console.log('[PAUSE] All timers paused - rotation and inactivity blocked');
    }
  });
}
...
manual_electron_update() {
    ...
    if [[ -z "$CURRENT_ELECTRON" ]]; then
        ...
        CURRENT_ELECTRON="unknown"
    fi

    echo "Current Electron: v${CURRENT_ELECTRON}"
    echo ""
    ...
    if [[ -z "$LATEST_ELECTRON" ]]; then
        log_error "Could not fetch latest Electron version from npm"
        echo "Check your internet connection"
        pause
        return 1
    fi

    echo "Latest Electron:  v${LATEST_ELECTRON}"
    echo ""
    ...
    if [[ "$CURRENT_ELECTRON" != "unknown" ]]; then
    ...
    read -r -p "Update to v${LATEST_ELECTRON}? (y/n): " DO_UPDATE
    ...
    if sudo -u "$KIOSK_OWNER" mkdir -p "$BACKUP_DIR" 2>/dev/null || mkdir -p "$BACKUP_DIR"; then
        ...
        log_error "Could not create backup directory"
        read -r -p "Continue without backup? (y/n): " CONTINUE_NO_BACKUP
    ...
    # Perform update
    echo "Updating Electron to v${LATEST_ELECTRON}..."
    ...
    if cd "$ACTUAL_KIOSK_DIR"; then
        ...
        if [[ $UPDATE_STATUS -eq 0 ]]; then
            echo ""
            log_success "Electron updated successfully to v${LATEST_ELECTRON}!"
            echo ""
            ...
            if [[ "$NEW_VERSION" == "$LATEST_ELECTRON" ]]; then
                ...
                echo "⚠ Version verification inconclusive"
            fi
            echo ""

            read -r -p "Restart kiosk now to use new version? (y/n): " RESTART_NOW
            ...
            echo ""
            log_error "Update failed (exit code: $UPDATE_STATUS)"
            echo ""
            echo "To rollback, see instructions above"
