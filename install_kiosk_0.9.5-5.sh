#!/bin/bash
################################################################################
###   Ubuntu Based Kiosk (UBK) v0.9.5-5          ###
################################################################################
#
# RELEASE v0.9.5-5 - CRITICAL Boot Password Bypass Fix
#
# What's NEW in v0.9.5-5:
# - CRITICAL SECURITY FIX: Ctrl+Alt+Del now blocked when system is locked out
#   * Power menu (Shutdown/Restart/Reload) no longer accessible during boot password
#   * Added isLockedOut check to showPowerMenu() function
#   * Prevents bypassing boot password requirement via Reload option
#   * Prevents bypassing inactivity lockout and time-based lockout
# - All fixes from v0.9.5-4 included
#
...
function showPowerMenu(){
  // Prevent power menu from showing when locked out (including boot password)
  if(isLockedOut){
    console.log('[SECURITY] Power menu blocked - system is locked out');
    return;
  }
