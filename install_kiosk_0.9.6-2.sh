...
#   * Simple pairing interface via addon menu
# - NEW: Bluetooth device management
#   * List, pair, unpair, and trust bluetooth devices
#   * Auto-connect on boot for trusted devices
#   * Battery status for supported devices
# - REMOVED: WebRTC intercom system (use talkkonnect instead)
# - REMOVED: Jitsi web intercom (use talkkonnect instead)
# - RECOMMENDED: Use talkkonnect addon for native PTT intercom
#   * Better audio quality than web-based solutions
#   * Lower latency
#   * Works with Bluetooth PTT buttons
# - All fixes from v0.9.5-12 included
#
# RELEASE v0.9.5-12 - Site-Specific Extension Fix
#
# What was in v0.9.5-12:
# - CRITICAL FIX: Time extensions now properly cleared on automatic rotation
#   * ISSUE: Extensions granted on one site would persist when automatically rotating to another site
#     Example: User gets 15-min extension on Site A, Site A's timer expires and rotates to Site B,
#     Site B would stay paused for the remaining extension time (incorrect behavior)
#   * ROOT CAUSE: rotateToNextSite() function didn't clear inactivityExtensionUntil
#     Manual navigation (Ctrl+Tab, swipe) already cleared extensions, but automatic rotation didn't
#   * FIX: Added inactivityExtensionUntil=0 in rotateToNextSite() function (line 4537)
#     Extensions now site-specific: when navigating away (manually OR automatically), rotation resumes
#   * BEHAVIOR: Extensions only apply to the site they were granted on
#     If user grants 30-min extension to read a recipe, finishes in 20 min, and navigates away,
#     the new site will rotate normally (extension doesn't follow to new site)
# - All fixes from v0.9.5-11 included
#
# RELEASE v0.9.5-11 - BrowserView Attachment Fix
#
# What was in v0.9.5-11:
# - CRITICAL FIX: Resolved BrowserView attachment error on password unlock
#   * ERROR: When unlocking password screen, users received error:
#     "Error: Given BrowserView is not attached to the window"
#     This caused a white screen requiring Ctrl+Alt+Del → Reload to recover
#   * ROOT CAUSE: showLockoutScreen() removes all BrowserViews (security feature)
#     but unlockScreen() called setTopBrowserView() without first adding the view back
#     Electron requires view to be attached before setting as top
#   * FIX: Added mainWindow.addBrowserView() before setTopBrowserView() in attachView()
#     function (line 4538-4544), wrapped in try-catch for already-attached cases
# - All fixes from v0.9.5-10 included
#
...
first_time_install() {
    ...
    echo "  • talkkonnect/Mumble intercom"
    ...
    echo "[7-11/27] Core configuration..."
    configure_touch_controls
    configure_navigation_security
    configure_optional_features
    configure_password_protection
...
addon_bluetooth() {
    ...
    echo "═══════════════════════════════════════════════════════════════"
    echo
    echo "Bluetooth allows wireless connection to:"
    echo "  • Speakers and headphones (audio output)"
    echo "  • Bluetooth headsets (audio input/output)"
    echo "  • PTT buttons and remote controls"
    echo
    
    # Ensure bluetooth packages are installed
    ...
    if systemctl is-active --quiet bluetooth; then
        echo "Status: ✓ Bluetooth service running"
        echo
        ...
        if bluetoothctl devices 2>/dev/null | grep -q "Device"; then
            bluetoothctl devices | while read -r line; do
                ...
                dev_name=$(echo "$line" | sed 's/^Device [^ ]* //')
                # Check if connected
                if bluetoothctl info "$dev_mac" 2>/dev/null | grep -q "Connected: yes"; then
                    echo "  ✓ $dev_name ($dev_mac) - Connected"
                else
                    echo "    $dev_name ($dev_mac) - Paired"
                fi
            done
        else
        ...
        echo "Status: ✗ Bluetooth service not running"
        echo
    fi
    
    echo "Options:"
    echo "  1. Scan and pair new device"
    echo "  2. Remove paired device"
    echo "  3. Connect to paired device"
    echo "  4. Disconnect device"
    echo "  5. Show device info"
    echo "  6. Enable/disable bluetooth"
    echo "  0. Return"
    echo
    read -r -p "Choose [0-6]: " bt_choice
    ...
    case "$bt_choice" in
        ...
        2)
            ...
            echo
            read -r -p "Enter device MAC address to remove: " bt_mac
            if [[ -n "$bt_mac" ]]; then
        ...
        3)
            ...
            bluetoothctl devices
            echo
            read -r -p "Enter device MAC address to connect: " bt_mac
            if [[ -n "$bt_mac" ]]; then
        ...
        4)
            ...
            bluetoothctl devices
            echo
            read -r -p "Enter device MAC address to disconnect: " bt_mac
            if [[ -n "$bt_mac" ]]; then
        ...
        5)
            ...
            bluetoothctl devices
            echo
            read -r -p "Enter device MAC address for info: " bt_mac
            if [[ -n "$bt_mac" ]]; then
...
view_logs() {
    ...
    echo "  4. Bluetooth log (last 50 lines)"
    echo "  0. Return"
    echo
    ...
    case "$choice" in
        ...
        4) sudo journalctl -u bluetooth -n 50; pause ;;
        0) return ;;
    esac
}
