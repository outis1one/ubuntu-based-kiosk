#!/bin/bash
################################################################################
###   Ubuntu Based Kiosk (UBK) v0.9.8.3        ###
################################################################################
#
# RELEASE v0.9.8.3 - UI & Performance Fixes
#
# What's new in v0.9.8.3:
# - UI improvements: Replaced box characters (╔ ╗) with simple equals (=)
# - Performance: Optimized addon loading (removed slow systemctl calls)
# - Added password lock configuration to initial setup wizard
# - Fixed RTR popup requiring Home URL (now works with or without Home URL)
# - Fixed time extensions carrying over when switching tabs
#   * Extensions are now site-specific and clear on tab switch
#   * 15-min extension on Site 1 won't affect Site 2
#
# What's in v0.9.8:
