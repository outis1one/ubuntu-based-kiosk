#!/bin/bash
################################################################################
###   Ubuntu Based Kiosk (UBK) v0.9.4          ###
################################################################################
#
# RELEASE v0.9.4 - Optional Features & Security
#
# What's NEW in v0.9.4:
# - NEW: Optional pause button (can be disabled during install/rerun)
# - NEW: Optional keyboard button (can be disabled during install/rerun)
# - NEW: Password protection with customizable session lockout
# - Lockout displays blank screen with no interaction allowed
# - Password required after display schedule wake-up
# - All features configurable during first install or rerun
#
...
function unlockScreen(){
  ...
  // Show current view
  if(showingHidden){
    if(hiddenViews[currentHiddenIndex]&&!hiddenViews[currentHiddenIndex].isDestroyed()){
      hiddenViews[currentHiddenIndex].show();
    }
  }else{
    if(views[currentIndex]&&!views[currentIndex].isDestroyed()){
...
core_menu() {
    while true; do
        ...
        case "$choice" in
            ...
            7) configure_optional_features; save_config ;;
            8) configure_password_protection; save_config ;;
