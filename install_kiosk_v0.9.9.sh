...
################################################################################
#
# RELEASE v0.9.9 - Easy Asterisk Intercom Integration
# - Added Easy Asterisk Intercom addon with automatic updates
...
download_and_install_easy_asterisk() {
    ...
    if sudo bash "$temp_script"; then
        ...
        sudo mkdir -p "$EASY_ASTERISK_INSTALL_DIR"
        echo "$version" | sudo tee "$EASY_ASTERISK_VERSION_FILE" > /dev/null
...
addon_easy_asterisk_intercom() {
    ...
    if [ -n "$installed_version" ]; then
        ...
        # Initialize backup_dir as empty for first install
        local backup_dir=""
    fi
